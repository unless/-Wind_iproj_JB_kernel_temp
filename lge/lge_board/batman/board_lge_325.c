...
#ifdef CONFIG_LGE_SILENT_RESET_PATCH // silent_reset_fusion2
#define LGE_SILENT_RESET_SIZE (4 * SZ_1K)
#endif
...
#ifdef CONFIG_LGE_SILENT_RESET_PATCH // silent_reset_fusion2
static struct resource silent_reset_resource[] = {
	{
		.name = "silent_reset",
		.flags = IORESOURCE_MEM,
	}
};
static struct platform_device silent_reset_device = {
	.name = "silent_reset",
	.id = -1,
	.num_resources = ARRAY_SIZE(silent_reset_resource),
	.resource = silent_reset_resource,
};

void __init lge_add_silent_reset_devices(void) // silent_reset_fusion2
{
	struct resource* res = silent_reset_resource;
	res->start = allocate_contiguous_ebi_nomap(LGE_SILENT_RESET_SIZE, SZ_4K);
	res->end = res->start + LGE_SILENT_RESET_SIZE - 1;

	printk(KERN_INFO "silent_reset_device START ADDR : 0x%x\n", res->start);
	printk(KERN_INFO "silent_reset_device END ADDR   : 0x%x\n", res->end);

	platform_device_register(&silent_reset_device);
}
#endif
...
static struct touch_device_caps touch_caps = {
	...
	.button_name 				= {KEY_BACK,KEY_HOME,KEY_SEARCH,KEY_MENU},
#else
...
#if defined(CONFIG_MACH_LGE_325_BOARD_VZW)
int dload_first_boot = 0;

static int __init dload_first_boot_setup(char *check)
{
...
__setup("lge.dload_first_boot=", dload_first_boot_setup);


int vs950_need_touch_downlaod = 0;
static int __init need_touch_download_setup(char *ntd)
{
	if(!strcmp(ntd, "1"))
		vs950_need_touch_downlaod = 1;
	else
		vs950_need_touch_downlaod = 0;

	printk("%s: need touch download %d", __func__, vs950_need_touch_downlaod);
	return 1;
}
__setup("lge.ntd=", need_touch_download_setup);
#endif
/*
 * Consumer specific regulator names:
 *			 regulator name		consumer dev_name
...
#ifdef CONFIG_LGE_SILENT_RESET_PATCH
static void __init reserve_silent_reset_memory(void)
{
#if 1 // silent_reset_fusion2
	msm8x60_reserve_table[MEMTYPE_EBI1].size += LGE_SILENT_RESET_SIZE;
#endif
}
...
#define PM8058_GPIO_INT           88
	...
	[0] = {
		.name		= "button-backlight",//"keypad:drv",
		.max_brightness = 5,	    // DCM HW request, 2012-0412, key led brightness 4mA
		.id		= PMIC8058_ID_LED_2,
	},
#else
		[0] = {
...
static void lge325_touch_i2c_register(void)
{
	...
	touch_caps.x_max		=  ((lge_bd_rev < LGE_REV_D) ? 1535 : 1535);	// 1572
	touch_caps.y_max		=  ((lge_bd_rev < LGE_REV_D) ? 2047 : 2047);	// 2244
