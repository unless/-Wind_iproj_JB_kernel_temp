	...
	struct synaptics_fw_data *fw, struct synaptics_ts_data *ts)
{
	...
	if (ret < 0) {
		TOUCH_ERR_MSG("SynaWaitForATTN 2000ms(Temp for dcm ics) timeout error\n");
		return ret;
	}
...
static int RMI4ProgramConfiguration(struct synaptics_fw_data *fw, struct synaptics_ts_data *ts)
{
	...
	for (blockNum = 0; blockNum < fw->m_configBlockCount; blockNum++) {
		...
		if (touch_debug_mask & DEBUG_FW_UPGRADE)
			TOUCH_DEBUG_MSG("uData=%x %x , puData=0x%x, m_configBlockCount=%d\n",
						uData[0], uData[1], *puData, fw->m_configBlockCount);
		...
		struct synaptics_fw_data *fw, struct synaptics_ts_data *ts)
{
	...
	return 1; //fw->m_firmwareImgVersion != 0 || bootloadID == fw->m_BootloadID;
}
