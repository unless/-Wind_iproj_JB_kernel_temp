...
static int resume_mv_set(const char *val, struct kernel_param *kp)
{
	...
	if (!rc)
		rc = pm8xxx_batt_alarm_threshold_set(
			PM8XXX_BATT_ALARM_UPPER_COMPARATOR, (4350-(lge_battery_info*150)));
#else
	rc = pm8xxx_batt_alarm_threshold_set(
...
#ifdef CONFIG_LGE_PM_BATTERY_ALARM	/*                                                       */
/*                                */
int is_lge_charger_inited(void)
{
    return pm8058_chg.inited;
}
EXPORT_SYMBOL(is_lge_charger_inited);
/*                                */
...
#endif
#else
  printk(KERN_DEBUG "############ Handle Event: [pm8058_chg_battconnect_handler] #####################\n");
  msm_charger_notify_event(&usb_hw_chg, CHG_BATT_REMOVE_EVENT);
#endif
#endif
	return IRQ_HANDLED;
}
...
/*                                                                                                 
    ...
    if (!is_chg_plugged_in()) {
		...
		if (rc > P15_THRESHOLD_LOWER(lge_battery_info))
			threshold_mv = P15_THRESHOLD_LOWER(lge_battery_info);
		else if (rc > P03_THRESHOLD_LOWER(lge_battery_info) && rc <= P15_THRESHOLD_LOWER(lge_battery_info))
			threshold_mv = P03_THRESHOLD_LOWER(lge_battery_info);
		else if (rc > P01_THRESHOLD_LOWER(lge_battery_info) && rc <= P03_THRESHOLD_LOWER(lge_battery_info))
			threshold_mv = P01_THRESHOLD_LOWER(lge_battery_info);
		else
			threshold_mv = P00_THRESHOLD_LOWER(lge_battery_info);
        ...
        rc = pm8xxx_batt_alarm_threshold_set(PM8XXX_BATT_ALARM_LOWER_COMPARATOR,
                AUTO_CHARGING_RESUME_MV_CALC(lge_battery_info));
        if (rc) {
